...
from src.services.document_processor_vectorized import process_documents_batch, get_processing_status
...
async def ask_question(
    ...
    try:
        ...
        references = []
        context_chunks = []
        page_info = []  # (doc_id, page_number) tuples
        ...
        if results['documents'] and len(results['documents'][0]) > 0:
            for i, doc_text in enumerate(results['documents'][0]):
                ...
                # Collect page information to fetch images later
                if doc_id and page_num != '?':
                    page_info.append((doc_id, page_num))
        ...
        # Fetch images from pages of retrieved chunks
        images_base64 = []
        if page_info:
            unique_pages = list(set(page_info))
            logger.info(f"Fetching images from {len(unique_pages)} unique pages")

            # Build query to get images from specific document pages
            for doc_id, page_num in unique_pages:
                db_images = db.query(DocumentImage).filter(
                    DocumentImage.document_id == doc_id,
                    DocumentImage.page_number == page_num
                ).all()

                images_base64.extend([img.image_data for img in db_images if img.image_data])

            logger.info(f"Loaded {len(images_base64)} images from relevant pages")
